...
if ($wupdated -eq 1) {
	if ($hikareboot -eq 1) {
		Show-Branding
		Write-Host "Your system have reached the desired UBR." -ForegroundColor Black -BackgroundColor Green -n; Write-Host " If there were no additional updates, you can press CTRL+C to leave Windows Update mode within 10 seconds." -ForegroundColor Cyan -n; Write-Host ([char]0xA0)
		Show-WaitTime 10
		Set-ItemProperty -Path "HKCU:\Software\AutoIDKU" -Name "Hikareboot" -Value 0 -Type DWord -Force
		exit
	} else {
		Show-Branding
		Write-Host "Your system have reached the desired UBR." -ForegroundColor Black -BackgroundColor Green -n; Write-Host " If you wish to continue updating, press CTRL+C within 10 seconds." -ForegroundColor Cyan -n; Write-Host ([char]0xA0)
		Show-WaitTime 10
	...
	Write-Host "To interrupt Windows Update mode, press CTRL+C within $sec seconds." -ForegroundColor Black -BackgroundColor Cyan -n; Write-Host ([char]0xA0)
	Show-WaitTime $sec
	Set-ItemProperty -Path "HKCU:\Software\AutoIDKU" -Name "Hikancel" -Value 0 -Type DWord -Force
	exit
}
